...
void FillHeaderInfo(Panel hSpecHud)
{
	...
	static char buf[64];
	Format(buf, sizeof(buf), "服务器: %s [位数 %i/%i | %iT]", sHostname, GetRealClientCount(), iMaxPlayers, iTickrate);
...
void FillSurvivorInfo(Panel hSpecHud)
{
	...
	switch (g_Gamemode)
	{
		case GAMEMODE_SCAVENGE:
		{
			...
			FormatEx(info, sizeof(info), "->1. 生还者 [%d胜 / %d局]", score, iRoundLimit);
		}
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					Format(info, sizeof(info), "%s: %iHP (#%s) [%s]", name, health, (incapCount == 2 ? "2倒" : "1倒"), info);
				}
			}
		}
...
void FillScoreInfo(Panel hSpecHud)
{
	...
	switch (g_Gamemode)
	{
		case GAMEMODE_SCAVENGE:
		{
			...
			FormatEx(info, sizeof(info), "> 累积时间 [%02d:%02.0f]", iMinutes, fDuration - 60 * iMinutes);
			DrawPanelText(hSpecHud, info);
			...
			if (bSecondHalf)
			{
				...
				FormatEx(info, sizeof(info), "> 对手用时 [%02d:%05.2f]", iMinutes, fDuration - 60 * iMinutes);
				DrawPanelText(hSpecHud, info);
			}
		}
		...
		case GAMEMODE_VERSUS:
		{
			if (bHybridScoremod)
			{
				...
				FormatEx(info, sizeof(info), "> 奖励分: %i <%.1f%%>", totalBonus, L4D2Util_IntToPercentFloat(totalBonus, maxTotalBonus));
				DrawPanelText(hSpecHud, info);
			...
			else if (bScoremod)
			{
				...
				FormatEx(info, sizeof(info), "> 血量奖励分: %i", healthBonus);
				DrawPanelText(hSpecHud, info);
			...
			else if (bNextScoremod)
			{
				...
				FormatEx(	info,
							sizeof(info),
							"> Perm: %i | Temp: %i | 药分: %i",
							permBonus, tempBonus, pillsBonus);		// i dont know what is perm and temp when translating xd
				DrawPanelText(hSpecHud, info);
...
void FillInfectedInfo(Panel hSpecHud)
{
	...
	switch (g_Gamemode)
	{
		case GAMEMODE_SCAVENGE:
		{
			...
			FormatEx(info, sizeof(info), "->2. 感染者 [%d胜 / %d局]", score, iRoundLimit);
		}
		case GAMEMODE_VERSUS:
		{
		...
		else
		{
			...
			else
			{
				...
				if (GetEntityFlags(client) & FL_ONFIRE)
				{
					...
					FormatEx(info, sizeof(info), "%s: %s (%iHP) [着火]%s", name, zClassName, iHP, buffer);
				}
				else
				{
...
bool FillTankInfo(Panel hSpecHud, bool bTankHUD = false)
{
	...
	switch (passCount)
	{
		case 0: FormatEx(info, sizeof(info), "原控");
		case 1: FormatEx(info, sizeof(info), "%i控", passCount);
		case 2: FormatEx(info, sizeof(info), "%i控", passCount);
		case 3: FormatEx(info, sizeof(info), "%i控", passCount);
		default: FormatEx(info, sizeof(info), "%i控", passCount);
	}

	if (!IsFakeClient(tank))
	{
		GetClientFixedName(tank, name, sizeof(name));
		Format(info, sizeof(info), "控制人 : %s (%s)", name, info);
	}
	else
	{
		Format(info, sizeof(info), "控制人 : AI (%s)", info);
	}
	DrawPanelText(hSpecHud, info);
	...
	if (!IsFakeClient(tank))
	{
		FormatEx(info, sizeof(info), "网络情况: %ims / %.1f", RoundToNearest(GetClientAvgLatency(tank, NetFlow_Both) * 1000.0), LM_GetLerpTime(tank) * 1000.0);
	}
	else
	{
		info = "网络情况: AI";
	...
	if (GetEntityFlags(tank) & FL_ONFIRE)
	{
		...
		FormatEx(info, sizeof(info), "着火 : %is", timeleft);
		DrawPanelText(hSpecHud, info);
	}
...
void FillGameInfo(Panel hSpecHud)
{
	...
	switch (g_Gamemode)
	{
		case GAMEMODE_SCAVENGE:
		{
			...
			FormatEx(info, sizeof(info), "%i局内胜出", iRoundLimit);
			DrawPanelText(hSpecHud, info);
		}
		...
		case GAMEMODE_VERSUS:
		{
			...
			if (l4d_tank_percent != null && l4d_witch_percent != null)
			{
				...
				if (iTankCount > 0)
				{
					...
					{
						FormatEx(info, sizeof(info), "Tank: %s", (bStaticTank ? "固定" : "事件"));
					}
				}
				...
				if (iWitchCount > 0)
				{
					...
					if (bDivide) {
						Format(info, sizeof(info), "%s | Witch: %s", info, ((bRoundHasFlowWitch || bCustomBossSys) ? buffer : (bStaticWitch ? "固定" : "事件")));
					} else {
						bDivide = true;
						FormatEx(info, sizeof(info), "Witch: %s", ((bRoundHasFlowWitch || bCustomBossSys) ? buffer : (bStaticWitch ? "固定" : "事件")));
					}
				}
