...
{    
    ...
    if (! IsFakeClient(tank_index)) 
    {
        ...
        Format(Buffer, sizeof(Buffer), "%t", "RageRefilledHintText");
        PrintHintText(tank_index, Buffer);
        for (int i = 1; i <= MaxClients; i++) 
        {
            if (! IsClientInGame(i) || GetClientTeam(i) != 3)
                continue;

            if (tank_index == i) CPrintToChat(i, "%t", "RageRefilledText");
            else CPrintToChat(i, "%t", "RageRefilledTextOthers", tank_index);
        }
...
public void outputTankToAll(any data)
{
    ...
    if (tankClientId != -1)
    {
        ...
        {
            CPrintToChatAll("%t", "TankSelection", tankClientName);
        }
        else
        {
            for (int i = 1; i <= MaxClients; i++) 
            {
                ...
                if (tankClientId == i) CPrintToChat(i, "%t", "YouAreTheTank");
