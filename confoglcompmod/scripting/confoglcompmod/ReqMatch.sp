...
static void RM_Match_Load()
{
	...
	CPrintToChatAll("%t", "MatchModeLoaded");
	...
	if (!RM_bIsMapRestarted && RM_hDoRestart.BoolValue)
	{
		...
		if (strlen(sMap) > 0)
		{
			CPrintToChatAll("%t", "ChangingMap", sMap);
		}
		else {
			GetCurrentMap(sMap, sizeof(sMap));

			// PrintToChatAll("\x01[\x05Confogl\x01] Restarting map!");
			CPrintToChatAll("%t", "RestartingMap");
		}
...
static void RM_Match_Unload(bool bForced = false)
{
	...
	CPrintToChatAll("%t", "MatchModeUnloaded");
...
static bool RM_UpdateCfgOn(const char[] cfgfile, bool bIsPrint = true)
{
	if (SetCustomCfg(cfgfile))
	{
		...
		CPrintToChatAll("%t", "LoadingConfig", cfgfile);
	...
	if (bIsPrint)
	{
		...
		CPrintToChatAll("%t", "ConfigNotFoundUsingDefault", cfgfile);
	}
...
public Action RM_Cmd_ForceMatch(int client, int args)
{
	...
	if (args < 1)
	{
		...
		if (client == 0)
		{
			...
			CPrintToChat(client, "%t", "SpecifyConfig");
		}
		return Plugin_Handled;
	}
	...
	if (!RM_UpdateCfgOn(sBuffer, false))
	{
		if (client == 0)
		{
			...
			CPrintToChat(client, "%t", "ConfigNotFound", sBuffer);
		}
	...
	if (args == 2)
	{
		...
		if (FindMap(sMap, sDisplayName, sizeof(sDisplayName)) == FindMap_NotFound)
		{
			if (client == 0)
			{
				...
				CPrintToChat(client, "%t", "MapNotFound", sMap);
			}
			return Plugin_Handled;
		}
