...
public Action MI_KV_CmdSave(int client, int args)
{
	...
	if (kMIData.JumpToKey(sCurMap, true)) {
		...
		ReplyToCommand(client, "%t", "AddedMapInfo", sCurMap, sNameBuff);
	}
...
public Action MI_KV_CmdSaveLoc(int client, int args)
{
	...
	if (!iIsInEditMode[client]) {
		if (!args) {
			ReplyToCommand(client, "%t", "EnterPointType");
			return Plugin_Handled;
		}
		...
		if (strcmp(sBuffer, "start_point", true) == 0) {
			...
			ReplyToCommand(client, "%t", "SetStartPoint");
		} else if (strcmp(sBuffer, "end_point", true) == 0) {
			iIsInEditMode[client] = 2;
			ReplyToCommand(client, "%t", "SetEndPoint");
		} else {
			ReplyToCommand(client, "%t", "SelectType");
			return Plugin_Handled;
		}
		...
		ReplyToCommand(client, "%t", "SetStartExtraPoint");
	...
	if (updateinfo) {
		...
		ReplyToCommand(client, "%t", "UpdatedMapInfo");
	}
