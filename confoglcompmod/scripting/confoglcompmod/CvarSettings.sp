...
public Action CVS_CvarSettings_Cmd(int client, int args)
{
	...
	if (!bTrackingStarted) {
		ReplyToCommand(client, "%t", "WaringCVarTracking");
		return Plugin_Handled;
	}
	...
	ReplyToCommand(client, "%t", "EnforcedServerCVars", cvscount);
	...
	if ((offset + 20) < cvscount) {
		ReplyToCommand(client, "%t", "ToSeeMoreCVars", offset + 20);
	}
...
public void CVS_ConVarChange(ConVar hConVar, const char[] sOldValue, const char[] sNewValue)
{
	if (bTrackingStarted) {
		...
		CPrintToChatAll("%t", "ServerCVarTracked", sName, sOldValue, sNewValue);
	}
}
